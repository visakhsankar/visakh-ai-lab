"""MCP Playground — main interactive page."""
...
with st.sidebar:
    ...
    st.markdown(
        f'<div style="text-align:center;padding:10px;background:#EDE9FE;border:1px solid #C4B5FD;border-radius:8px"><span style="font-size:22px;font-weight:800;color:#7C3AED">{runs_left_display}</span><span style="font-size:12px;color:#6D28D9;font-weight:600"> / {MAX_RUNS if not st.session_state.author_mode else "∞"} runs</span></div>',
        unsafe_allow_html=True,
    )
...
# ─── Header ───────────────────────────────────────────────────────────────────
st.markdown('<h1 style="font-size:2.2rem;font-weight:800;color:#1e293b;margin-bottom:2px">🔌 MCP Playground</h1>', unsafe_allow_html=True)
st.markdown('<p style="font-size:16px;color:#64748b;margin-bottom:2px">Watch AI connect to the world — every protocol message exposed.</p>', unsafe_allow_html=True)
st.markdown('<p style="font-size:12px;color:#94a3b8;margin-bottom:0">by <a href="https://visakhsankar.com" style="color:#7C3AED;text-decoration:none;font-weight:600">Visakh Sankar</a></p>', unsafe_allow_html=True)
st.markdown("<br>", unsafe_allow_html=True)

# ─── What is MCP? Explainer ───────────────────────────────────────────────────
with st.expander("💡 What is MCP? — click to understand before you run", expanded=False):
    st.markdown(
        """
        **Model Context Protocol (MCP)** is an open standard (created by Anthropic) that gives AI models a universal way to connect to tools, databases, and APIs — without hardcoding anything.

        Think of it like **USB for AI tools**. Before MCP, every AI app had to wire up its own custom tool connections. With MCP, any AI host can plug into any tool server using the same protocol.

        #### The 3 phases that happen every time you run a task:

        | Phase | What happens |
        |---|---|
        | 🤝 **Handshake** (`initialize`) | Client and Server agree on protocol version and capabilities |
        | 🔍 **Discovery** (`tools/list`) | Client asks "what tools do you have?" — Server returns full list with schemas |
        | ⚡ **Execution** (`tools/call`) | AI decides which tool to call → Client routes it → Server runs it → result flows back |

        The **Protocol Inspector** below shows every single JSON-RPC 2.0 message from these phases.
        """
    )
...
if st.session_state.last_answer:
    ...
    # Annotated protocol messages with plain-English labels
    phase_labels = {
        "initialize": ("🤝 Phase 1: Handshake", "Client introduces itself to the server and agrees on protocol version"),
        "tools/list": ("🔍 Phase 2: Discovery", "Client asks the server what tools are available"),
        "tools/call": ("⚡ Phase 3: Tool Execution", "AI decided to call a tool — routed through MCP"),
    }

    last_phase = None
    for msg in log:
        method = msg.get("method", "")
        if msg.get("direction") == "request" and method in phase_labels:
            phase_key, phase_desc = phase_labels[method]
            if phase_key != last_phase:
                st.markdown(
                    f'<div style="margin:20px 0 6px;padding:8px 14px;background:#F5F3FF;border-left:4px solid #7C3AED;border-radius:0 6px 6px 0"><span style="font-size:12px;font-weight:700;color:#7C3AED">{phase_key}</span><br><span style="font-size:11px;color:#64748b">{phase_desc}</span></div>',
                    unsafe_allow_html=True,
                )
